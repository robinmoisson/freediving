...
var GoogleSound = (function () {
    ...
    /**
     * This is useful for bypassing restriction from android chrome browser, preventing autoplay. The user must initaite
     * an action with the page before we can play any sound - but if the action only loads the first sound ('breathe',
     * for ex), all others are considered non safe to play by the browser.
     *
     * The function touches all sounds so all of them are considered triggered by a user action.
     */
    googleSound.prototype.preloadSounds = function () {
        $.each(_allSounds, function(key, sound) {
            sound.load();
        })
    };
